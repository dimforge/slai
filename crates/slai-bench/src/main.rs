...
use slai::quantized_matrix::GpuQuantMatrix;
use stensor::shapes::ViewShapeBuffers;
use stensor::tensor::{GpuMatrix, GpuVector, TensorBuilder};
use slang_hal::Shader;
use stensor::{Gemm, GemmVariant, GemvVariant};
...
async fn main() -> anyhow::Result<()> {
    ...
    let gemv = GemvQuant::from_backend(gpu.backend())?;
    let gemm = Gemm::from_backend(gpu.backend())?;
    let dim = 512;
        ...
        macro_rules! bench_quantized(
            ($($scalar: ident, $quant: ty);*) => {$(
                let runtime = bench_gemv::<$quant>(&gpu, dim, dim, |pass, result, m, v| {
                    gemv.launch(gpu.backend(), &shapes, pass, result, m, v);
                }).await?;
                gemv_bench.push(
                    GemvBenchEntry {
                        matrix: (dim, dim),
    ...
    rand::distributions::Standard: rand::prelude::Distribution<T>,
{
    ...
    let mut timestamps = GpuTimestamps::new(gpu.backend(), 2);
    ...
    for step in 0..20 {
        ...
        if step > 10 {
            let timestamps = timestamps.wait_for_results_ms(gpu.backend(), gpu.queue());
...
async fn bench_gemm<'a, 'b: 'a>(
    ...
    let m1 = TensorBuilder::matrix(nrows, ncols, BufferUsages::STORAGE)
        .build_init(gpu.backend(), m1_cpu.as_slice());
    let m2 = TensorBuilder::matrix(ncols, nrows, BufferUsages::STORAGE)
        .build_init(gpu.backend(), m2_cpu.as_slice());
