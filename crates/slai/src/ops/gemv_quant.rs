...
pub struct GemvQuant<B: Backend> {
    pub gemv_f32: Gemv<B>,
    pub gemv_q8: GemvQ8_0x2<B>,
    pub gemv_q5: GemvQ5_0x2<B>,
    pub gemv_q4: GemvQ4_0x2<B>,
    pub gemv_q5_1: GemvQ5_1x2<B>,
    pub gemv_q4_1: GemvQ4_1x2<B>,
    pub gemv_q8_k: GemvQ8K<B>,
    pub gemv_q6_k: GemvQ6Kx2<B>,
    pub gemv_q5_k: GemvQ5K<B>,
    pub gemv_q4_k: GemvQ4K<B>,
}

impl<B: Backend> GemvQuant<B> {
    pub fn from_backend(backend: &B, compiler: &SlangCompiler) -> Result<Self, B::Error> {
        Ok(Self {
            gemv_f32: Gemv::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q5: GemvQ5_0x2::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q5_1: GemvQ5_1x2::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q4: GemvQ4_0x2::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q4_1: GemvQ4_1x2::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q8_k: GemvQ8K::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q6_k: GemvQ6Kx2::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q5_k: GemvQ5K::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q4_k: GemvQ4K::from_backend(backend, compiler).unwrap(), // ?,
            gemv_q8: GemvQ8_0x2::from_backend(backend, compiler).unwrap(), // ?,
        })
    }
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q8_0x2")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ8_0x2<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q5_0x2")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ5_0x2<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q5_1x2")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ5_1x2<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q4_0x2")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ4_0x2<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q4_1x2")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ4_1x2<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q8_k")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ8K<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q6_kx2")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ6Kx2<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q5_k")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ5K<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(Shader)]
#[shader(module = "slai::gemv_quant_q4_k")]
/// Shader for computing the product of a matrix and a vector.
pub struct GemvQ4K<B: Backend> {
    pub gemv: GpuFunction<B>,
}

#[derive(ShaderArgs)]
struct GemvQuantArgs<'a, B: Backend> {
    shape_out: &'a B::Buffer<ViewShape>,
    shape_m: &'a B::Buffer<ViewShape>,
    shape_v: &'a B::Buffer<ViewShape>,
    out: B::BufferSlice<'a, f32>,
    m: &'a GpuQuantMatrix<B>,
...
impl<B: Backend> GemvQuant<B> {
    ...
    pub fn launch<'a, 'b>(
        ...
        m: &GpuQuantMatrix<B>,
        v: impl Into<GpuTensorView<'b, f32, B>>,
    ) -> Result<(), B::Error> {
        let out = out.into();
        ...
        let v_shape = match m {
            ...
            GpuQuantMatrix::Q8_0(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q5_0(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q5_1(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q4_0(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q4_1(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q8K(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q6K(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q5K(_) => v.shape().f32_to_vec4(),
            GpuQuantMatrix::Q4K(_) => v.shape().f32_to_vec4(),
        };

        let optimized_shape = if USE_OPTIMIZED {
            out.shape().f32_to_vec4()
        } else {
            out.shape()
        };
        ...
        shapes.insert(backend, m.shape())?; // TODO: propagate error
        let shape_out = shapes.get(out_shape).unwrap();
        let shape_v = shapes.get(v_shape).unwrap();
        let shape_m = shapes.get(m.shape()).unwrap();
        ...
        kernel.launch_grid(backend, pass, &args, [launch, 1, 1])
    }
}
