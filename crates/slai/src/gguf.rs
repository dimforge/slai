...
impl GgufTensorData {
    ...
    pub fn to_gpu_matrix<B: Backend>(
        &self,
        backend: &B,
        nrows: usize,
        ncols: usize,
    ) -> Result<Option<GpuQuantMatrix<B>>, B::Error> {
                ...
                match self {
                    $(Self::$match_id(v) => {
                        if v.len() % 2 == 0 && ncols % $gpu_ty::DEQUANTIZED_LEN == 0 {
                            ...
                            return Ok(Some(GpuQuantMatrix::$match_id(GpuTensor::matrix_with_ordering(
                                backend,
                                &mat,
        ...
        self.dequantize()
            .map(|deq| {
                ...
                Ok(GpuQuantMatrix::F32(GpuTensor::matrix_with_ordering(
                    backend,
                    &mat,
                    MatrixOrdering::RowMajor,
                    BufferUsages::STORAGE,
...
impl GgufTensor {
    ...
    pub fn to_gpu_matrix<B: Backend>(
        ...
        let [nrows, ncols, nmats, ncubes] = self.dimensions;
        assert_eq!(nmats, 1);
        assert_eq!(ncubes, 1);
        self.data
            .to_gpu_matrix(backend, nrows as usize, ncols as usize)
    }
}
...
impl Gguf {
    ...
    fn load_tensors(
        ...
        for _ in 0..header.tensor_count {
            ...
            // GGUF stores ncols first, and then nrows.
            // Swap them so we have the more conventional `[nrows, ncols, ...]` representation.
            dimensions.swap(0, 1);
